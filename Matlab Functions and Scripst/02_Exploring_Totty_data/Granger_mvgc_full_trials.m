...
% Contextual Exposure
% temp_data{2,1} = data.lfp{7,1}; % CS or Time win Contextual Exposure

% Extinction ans Retrieval Sessions
%temp_data{1,1} = data.lfp{6,1}; % baseline
%temp_data{2,1} = data.lfp{7,1}; % CS or Time win Contextual Exposure
%temp_data{3,1} = data.lfp{8,1}; %  ITI

mvgc = [];

% Freezinf nd non-freezing epochs
temp_data{1,1} = data.lfp{6,1};  % baseline
temp_data{2,1} = data.lfp{7,1}; % freezing
temp_data{3,1} = data.lfp{8,1}; % non-freezing
...
for tt = 1:size(temp_data,1)
    ...
    if isempty(temp_data{tt,1})
        continue
    end

    if tt == 1 % case for baseline period

        for ii = 1:size(temp_data{tt,1},1)
            for jj = 1:size(temp_data{tt,1},3)
                mvgc.data{tt,1}(ii,:,jj) = zscore(decimate(temp_data{tt,1}(ii,B_clean{ms}(1):B_clean{ms}(2)-1,jj),mvgc.parameters.decimate_factor));
            end
        end
        ...
        for ii = 1:size(temp_data{tt,1},1)
            for jj = 1:size(temp_data{tt,1},3)
                mvgc.data{tt,1}(ii,:,jj) = zscore(decimate(temp_data{tt,1}(ii,:,jj),mvgc.parameters.decimate_factor));
            end
        end
    end
end
...
clear('B_clean','temp_data','tt','ii','jj')
...
mvgc.parameters.momax     = 35;                                                               % maximum model order for model order estimation
...
for tt = 1:size(mvgc.data,1)
    ...
    for ii = 1:size(mvgc.data{tt,1},3)
        ...
        ptic('\n*** tsdata_to_var... ');
        [mvgc.parameters.A{tt,ii},mvgc.parameters.SIG{tt,ii}] = tsdata_to_var(mvgc.data{tt,1}(:,:,ii),mvgc.parameters.morder,[]);
        assert(~isbad(mvgc.parameters.A{tt,ii}),'VAR estimation failed - bailing out');
        ptoc;
        ...
        mvgc.info{tt,ii} = var_info(mvgc.parameters.A{tt,ii},mvgc.parameters.SIG{tt,ii});
        assert(~mvgc.info{tt,ii}.error,'VAR error(s) found - bailing out');
...
for tt = 1:size(mvgc.data,1)
    ...
    for ii = 1:size(mvgc.data{tt,1},3)
        ...
        ptic('\n*** var_to_spwcgc... ');
        mvgc.F_spect{tt,ii} = var_to_spwcgc(mvgc.parameters.A{tt,ii},mvgc.parameters.SIG{tt,ii},mvgc.parameters.fres);
        assert(~isbad(mvgc.F_spect{tt,ii},false),'spectral GC calculation failed - bailing out');
        ptoc;
...
for cc = 1:size(Combinations_1,1)
        ...
        subplot(3,3,cc)
        plot(mvgc.parameters.freqs(mvgc.parameters.frex_idx_2_12Hz),10.*squeeze(mvgc.F_spect{1,1}(Combinations_1(cc,1),Combinations_1(cc,2),mvgc.parameters.frex_idx_2_12Hz)))
        hold
        plot(mvgc.parameters.freqs(mvgc.parameters.frex_idx_2_12Hz),10.*squeeze(mvgc.F_spect{1,1}(Combinations_2(cc,1),Combinations_2(cc,2),mvgc.parameters.frex_idx_2_12Hz)))
...
for cc = 1:size(Combinations_1,1)
        ...
        subplot(3,3,cc+3)
        plot(mvgc.parameters.freqs(mvgc.parameters.frex_idx_2_12Hz),10.*squeeze(data2plot_mean(Combinations_1(cc,1),Combinations_1(cc,2),mvgc.parameters.frex_idx_2_12Hz)))
        hold
        plot(mvgc.parameters.freqs(mvgc.parameters.frex_idx_2_12Hz),10.*squeeze(data2plot_mean(Combinations_2(cc,1),Combinations_2(cc,2),mvgc.parameters.frex_idx_2_12Hz)))

        if          cc == 1
                    legend('PL --> IL', 'IL --> PL')
                    ylabel({'Freezing';'Granger causality'},'FontWeight','bold','FontSize',14)
        elseif      cc == 2
                    legend('PL --> HPC', 'HPC --> PL')
        else
                    legend('IL --> HPC', 'HPC --> IL')
...
for cc = 1:size(Combinations_1,1)
        ...
        if          cc == 1
                    legend('PL --> IL', 'IL --> PL')
                    ylabel({'non-Freezing';'Granger causality'},'FontWeight','bold','FontSize',14)
...
for tt = 1:size(mvgc.data,1)
    ...
    for ii = 1:size(mvgc.data{tt,1},3)
        ...
        ptic('*** var_to_pwcgc... ');
        [mvgc.stats.F{tt,ii},mvgc.stats.pval{tt,ii}] = var_to_pwcgc(mvgc.parameters.A{tt,ii},mvgc.parameters.SIG{tt,ii},mvgc.data{tt,1},mvgc.parameters.regmode,mvgc.parameters.tstat);
        ptoc;
    end
end
...
for tt = 1:size(mvgc.data,1)
    ...
    for ii = 1:size(mvgc.data{tt,1},3)
        ...
        assert(~isbad(mvgc.stats.F{tt,ii},false),'GC calculation failed - bailing out');

        % Significance-test p-values, correcting for multiple hypotheses.

        mvgc.stats.sig{tt,ii} = significance(mvgc.stats.pval{tt,ii},mvgc.parameters.alpha,mvgc.parameters.mhtc);
...
Path    = files.FilesLoaded{1,1}(ms).folder;
newStr1 = id(1:end-8);
name_1 = strcat(Path,'/',newStr1,'_MVGC_Granger_freezing_epochs');
