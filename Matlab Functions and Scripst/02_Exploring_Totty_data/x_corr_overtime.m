function [xcorr_result_full,xcorr_result_full_lag_peak,xcorr_result_full_lag_peak_idx,xcorr_result,xcorr_result_lag_peaks,xcorr_lags] = x_corr_overtime(signal1, signal2, nlags, Fs, window_length, overlap, method,fp)
...
% Derived parameters
N = size(signal1,2); % Total number of samples
...
for start_idx = 1:step_size:N-window_samples
    ...
    [~, xcorr_lag_peak_idx] = max(xcorr_val);
...
% Full session
[xcorr_result_full,xcorr_lags] = xcorr(signal1, signal2, nlags, method);
% xcorr_lags = xcorr_lags * 1000/Fs;

% xcorr_result_full = normalize(mean(xcorr_result,1),'range');
