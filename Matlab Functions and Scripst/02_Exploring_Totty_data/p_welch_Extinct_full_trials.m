...
for jj = 1:size(pw.full_trial.Pxx,1)
    ...
    if jj == 1 % baseline condition
        pw.full_trial.Pxx_Baseline_norm_mean_first_trials{jj,1}   = pw.full_trial.Pxx_Baseline_norm{jj,1};   % baseline cell will in 1,1 of course....but just to keep the organization
        pw.full_trial.Pxx_TotalPower_norm_mean_first_trials{jj,1} = pw.full_trial.Pxx_TotalPower_norm{jj,1}; % baseline cell will in 1,1 of course....but just to keep the organization

        pw.full_trial.Pxx_Baseline_norm_mean_last_trials{jj,1}   = pw.full_trial.Pxx_Baseline_norm{jj,1};   % baseline cell will in 1,1 of course....but just to keep the organization
        pw.full_trial.Pxx_TotalPower_norm_mean_last_trials{jj,1} = pw.full_trial.Pxx_TotalPower_norm{jj,1}; % baseline cell will in 1,1 of course....but just to keep the organization

    else

        pw.full_trial.Pxx_Baseline_norm_mean_first_trials{jj,1}    = mean(pw.full_trial.Pxx_Baseline_norm{jj,1}(:,:,CSIT{ms}(1:size(CSIT{ms},2)/2)),3);   % averaging first 10 trials
        pw.full_trial.Pxx_TotalPower_norm_mean_first_trials{jj,1}  = mean(pw.full_trial.Pxx_TotalPower_norm{jj,1}(:,:,CSIT{ms}(1:size(CSIT{ms},2)/2)),3); % averaging first 10 trials

        pw.full_trial.Pxx_Baseline_norm_mean_last_trials{jj,1}    = mean(pw.full_trial.Pxx_Baseline_norm{jj,1}(:,:,CSIT{ms}((size(CSIT{ms},2)/2)+1:size(CSIT{ms},2))),3);   % averaging last 10 trials
        pw.full_trial.Pxx_TotalPower_norm_mean_last_trials{jj,1}  = mean(pw.full_trial.Pxx_TotalPower_norm{jj,1}(:,:,CSIT{ms}((size(CSIT{ms},2)/2)+1:size(CSIT{ms},2))),3); % averaging last 10 trials
...
data_2_plot_mean1 = pw.full_trial.Pxx_TotalPower_norm_mean_first_trials;
data_2_plot_mean2 = pw.full_trial.Pxx_TotalPower_norm_mean_last_trials;
...
for ii = 1:size(CSIT{ms},2)/2

    % First 10 trials
    t(ii+1) = subplot(6,12,ii+1);
    plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot{2, 1}(ch1,:,CSIT{ms}(1,ii)),'linewidth',3,'Color',[1 .6 .2])
    ...
    t(ii+25) = subplot(6,12,ii+25);
    plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot{2, 1}(ch2,:,CSIT{ms}(1,ii)),'linewidth',3,'Color',[1 .6 .2])
    ...
    t(ii+49) = subplot(6,12,ii+49);
    plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot{2, 1}(ch3,:,CSIT{ms}(1,ii)),'linewidth',3,'Color',[1 .6 .2])
    ...
    % Last 10 trials
    t(ii+13) = subplot(6,12,ii+13);
    plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot{2, 1}(ch1,:,CSIT{ms}(1,ii+size(CSIT{ms},2)/2)),'linewidth',3,'Color',[.5, .7, 1])
    ...
    axis('square')
    box off
    title({['CS-Tone ' num2str(CSIT{ms}(1,ii+size(CSIT{ms},2)/2))];[]})

    t(ii+37) = subplot(6,12,ii+37);
    plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot{2, 1}(ch2,:,CSIT{ms}(1,ii+size(CSIT{ms},2)/2)),'linewidth',3,'Color',[.5, .7, 1])
    ...
    t(ii+61) = subplot(6,12,ii+61);
    plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot{2, 1}(ch3,:,CSIT{ms}(1,ii+size(CSIT{ms},2)/2)),'linewidth',3,'Color',[.5, .7, 1])
...
m(1) = subplot(6,12,[12 24]);
plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot_mean1{2, 1}(ch1,:),'linewidth',3,'color',[1 .6 .2] )
hold on
plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot_mean2{2, 1}(ch1,:),'linewidth',3,'color',[.5, .7, 1] )
...
m(2) = subplot(6,12,[36 48]);
plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot_mean1{2, 1}(ch2,:),'linewidth',3,'color',[1 .6 .2] )
hold on
plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot_mean2{2, 1}(ch2,:),'linewidth',3,'color',[.5, .7, 1] )
...
m(3) = subplot(6,12,[60 72]);
plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot_mean1{2, 1}(ch3,:),'linewidth',3,'color',[1 .6 .2] )
hold on
plot(pw.full_trial.freq_baseline(pw.full_trial.parameters.frex_idx_2_12Hz),data_2_plot_mean2{2, 1}(ch3,:),'linewidth',3,'color',[.5, .7, 1] )
