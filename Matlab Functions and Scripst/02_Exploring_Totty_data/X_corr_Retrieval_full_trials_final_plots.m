...
for ii = 1:size(x_corr.result,1)
    for jj = 1:size(x_corr.result,2)
        ...
        result_full{ii,jj}(:,:,:,:,ms)                    = normalize(x_corr.result_full{ii,jj},2,'range');
        result{ii,jj}(:,:,:,:,ms)                         = normalize(x_corr.result{ii,jj},2,'range');
        result_full_trials_mean{ii,jj}(:,:,:,:,ms)        = normalize(x_corr.result_full_trials_mean{ii,jj},2,'range');
...
for ii = 1:size(x_corr.result,1)
    ...
    data_2_plot_result_full_trials_mean_mean_animals         = cellfun(@(x)mean(x,5),result_full_trials_mean,'UniformOutput',false);
...
for ii = 1:size(result_full,1)
    for jj = 1:size(result_full,2)
        ...
        if jj == 1 % baseline condition
            ...
            [~,data_2_plot_result_mean_animals_trial_peak_idx{jj,ii}] = max(data_2_plot_result_full_trials_mean_mean_animals{jj,ii},[],2);
...
for jj = 2:3
    ...
    subplot(5,2,2)
    boundedline(lags,data_2_plot_result_full_trials_mean_mean_animals{2,ff}(jj,:),data_2_plot_result_full_trials_mean_SEM_animals{2,ff}(jj,:),'LineWidth',2,'color',[.6 .6 .6],'transparency',.4);
    hold on
    plot(lags(data_2_plot_result_mean_animals_trial_peak_idx{2,ff}(jj,1)),data_2_plot_result_full_trials_mean_mean_animals{2,ff}(jj,data_2_plot_result_mean_animals_trial_peak_idx{2,ff}(jj,1)),'ro','MarkerEdgeColor','r','MarkerFaceColor','r');
...
% 
% name = strcat(path,'/','_x_corr_retrieval_7_10Hz');
% 
% %get all figures
