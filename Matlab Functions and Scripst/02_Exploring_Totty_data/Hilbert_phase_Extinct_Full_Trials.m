...
%% 10-Trials average. First 10 and last 10


hilb.delta_phase_10_trial = [];
hilb.delta_phase_mean_10_trial = [];
hilb.PLV_mean_10_trial = [];

for ii = 1:size(hilb.delta_phase,1)
    for jj = 1:size(hilb.delta_phase,2)
        
        if ii == 1
            hilb.delta_phase_10_trial{ii,jj}      = hilb.delta_phase{ii,jj};
            hilb.delta_phase_mean_10_trial{ii,jj} = hilb.delta_phase_mean_trial{ii,jj};
            hilb.PLV_mean_10_trial{ii,jj}         = hilb.PLV_mean_trial{ii,jj};
        else
            c = 1;
            for mm = 1:size(hilb.delta_phase{ii,jj},3)./2:size(hilb.delta_phase{ii,jj},3)                
                hilb.delta_phase_10_trial{ii,jj}(:,:,c)      = circ_mean(hilb.delta_phase{ii,jj}(:,:,mm:mm + (size(hilb.delta_phase{ii,jj},3)./2)-1),[],3);
                hilb.delta_phase_mean_10_trial{ii,jj}(:,c)   = circ_mean(hilb.delta_phase_mean_trial{ii,jj}(:,mm:mm + (size(hilb.delta_phase{ii,jj},3)./2)-1),[],2);
                hilb.PLV_mean_10_trial{ii,jj}(:,c)           = mean(hilb.PLV_mean_trial{ii,jj}(:,mm:mm + (size(hilb.delta_phase{ii,jj},3)./2)-1),2);
                c = c+1;
            end
        end
    end
...
% Define data to plot
data_phase = hilb.delta_phase_10_trial;

data_phase_mean = hilb.delta_phase_mean_10_trial  ;
data_PLV_mean = hilb.PLV_mean_10_trial ;
...
for ii = 1:size(data_phase,2)
    ...
    for tt = 1:size(data_phase{3,ii},3)
        ...
        if tt == 1
            color = [1 .6 .2];
            titl_ = '\fontsize{16} First 10 trials';
        else
            color = [.5, .7, 1];
            titl_ = '\fontsize{16} Last 10 trials';
        end
