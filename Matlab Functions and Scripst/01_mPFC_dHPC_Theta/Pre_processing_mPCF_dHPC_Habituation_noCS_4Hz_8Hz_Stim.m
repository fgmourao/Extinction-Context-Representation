...
% temp2 = data.lfp{5, 1}(9:16,:) - data.lfp{5, 1}(9,:);   % mPFC PL
% temp3 = data.lfp{5, 1}(17:32,:) - data.lfp{5, 1}(22,:); % dHPC
% 
% data.lfp{5, 1} = [];
...
%% Use first channel from each probe as Reference
% temp1 = data.lfp{5, 1}(1:8,:) - mean(data.lfp{5, 1}(1:32,:),1);    % mPFC IL
% temp2 = data.lfp{5, 1}(9:16,:) - mean(data.lfp{5, 1}(1:32,:),1);   % mPFC PL
% temp3 = data.lfp{5, 1}(17:32,:) - mean(data.lfp{5, 1}(1:32,:),1); % dHPC
% 
% data.lfp{5, 1} = [];
% data.lfp{5, 1} = [temp1;temp2;temp3];
...
% 
% Time_epoch = 30; %seconds
% 
% data.events{2,1}(:,1) = 1:Time_epoch*parameters.decimated_srate:length(data.lfp{5,1});
...
% Time in samples
data.events{2,1}(:,1) = cell2mat(dataascell(1,emptyCells==0))'; % CS-Start
...
% Organizing accordingly Optogenetic Stim Conterbalanced Order
if OptoSTIM{ms}(1,1) == 4
    data.events{3,1} = data.events{2,1}(1:5,:); % 4Hz CS
    data.events{3,2} = data.events{2,2}(1:5,:); % 4Hz ITI

    data.events{4,1} = data.events{2,1}(6:10,:); % 8Hz CS
    data.events{4,2} = data.events{2,2}(6:10,:); % 8Hz ITI

else
    data.events{3,1} = data.events{2,1}(6:10,:); % 4Hz CS
    data.events{3,2} = data.events{2,2}(6:10,:); % 4Hz ITI

    data.events{4,1} = data.events{2,1}(1:5,:); % 8Hz CS
    data.events{4,2} = data.events{2,2}(1:5,:); % 8Hz ITI
end
...
for ii = 1:size(data.lfp,2)
    for jj = 1:size(data.events{3, 1},1)
        ...
        data.lfp{7,ii}(:,:,jj) = data.lfp{5,ii}(:,(round((data.events{3, 1}(jj,1))./parameters.downsampling)):(round((data.events{3, 1}(jj,2))./parameters.downsampling)-1));
...
% 4 HZ ITI-Trials
for ii = 1:size(data.lfp,2)
    for jj = 1:size(data.events{3, 2},1)
            
        data.lfp{8,ii}(:,:,jj) = data.lfp{5,ii}(:,(round((data.events{3, 2}(jj,1))./parameters.downsampling)):(round((data.events{3, 2}(jj,2))./parameters.downsampling)-1));
...
% 8 HZ CS-Trials
for ii = 1:size(data.lfp,2)
    for jj = 1:size(data.events{4, 1},1)

        data.lfp{9,ii}(:,:,jj) = data.lfp{5,ii}(:,(round((data.events{4, 1}(jj,1))./parameters.downsampling)):(round((data.events{4, 1}(jj,2))./parameters.downsampling)-1));
...
% 8 HZ ITI-Trials
for ii = 1:size(data.lfp,2)
    for jj = 1:size(data.events{4, 2},1)
            
        data.lfp{10,ii}(:,:,jj) = data.lfp{5,ii}(:,(round((data.events{4, 2}(jj,1))./parameters.downsampling)):(round((data.events{4, 2}(jj,2))./parameters.downsampling)-1));

        if isempty(data.lfp{1,ii}) % Future raw data - The raw data has never been filtered until now.
            continue
        else
%             data.lfp{4,ii}(:,:,jj) = data.lfp{1,ii}(:,data.events{3, 2}(jj,1):data.events{4, 2}(jj,2)-1);
        end
    end
end
...
% 
name = strcat(path,'/',newStr,'_data_habituation');
