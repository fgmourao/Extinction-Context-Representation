...
hilb.parameters.filters = [3.5 5;7 9];
...
data_PLV_mean = hilb.PLV_mean_trial_bins;
...
%     hold on
%     I=polarplot([0 data_phase_mean{1,ii}(1,1)],[0 data_PLV_mean{1,1}(1,1)]);
...
%     title({['\fontsize{16} Baseline'];[];['\fontsize{14} PLV = ', num2str(data_PLV_mean{1,ii}(1,1))];[]})
    ...
    % mPFC PL <--> dHPC
    subplot(2,1,1)
    polarplot([zeros(1,size(data_phase{1,ii}(2,1:100:end),2)) data_phase{1,ii}(2,1:100:end)]',repmat([0 1],1,size(data_phase{1,ii}(2,1:100:end),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_mean{1,ii}(2,1)],[0 data_PLV_mean{1,1}(2,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{14} PLV = ', num2str(data_PLV_mean{1,1}(2,1))];[]})

    % mPFC IL <--> dHPC
    subplot(2,1,2)
    polarplot([zeros(1,size(data_phase{1,ii}(3,1:100:end),2)) data_phase{1,ii}(3,1:100:end)]',repmat([0 1],1,size(data_phase{1,ii}(3,1:100:end),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_mean{1,ii}(3,1)],[0 data_PLV_mean{1,1}(3,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{14} PLV = ', num2str(data_PLV_mean{1,1}(3,1))];[]})
...
%     title({['\fontsize{16} - 8 Hz Stimulation -'];[];['\fontsize{14} PLV = ' num2str(data_PLV_mean{3,ii}(1,1))];[]},'color',[0, 0, .6])
    ...
    % mPFC PL <--> dHPC
    subplot(3,3,6)
    polarplot([zeros(1,size(data_phase{3,ii}(2,1:200:end,:),2)) circ_mean(data_phase{3,ii}(2,1:200:end,:),[],3)]',repmat([0 1],1,size(data_phase{3,ii}(2,1:200:end,:),2))','color',[.6 .6 .6]);
    hold on
    ...
    % mPFC IL <--> dHPC
    subplot(3,3,9)
    polarplot([zeros(1,size(data_phase{3,ii}(3,1:200:end,:),2)) circ_mean(data_phase{3,ii}(3,1:200:end,:),[],3)]',repmat([0 1],1,size(data_phase{3,ii}(3,1:200:end,:),2))','color',[.6 .6 .6]);
    hold on
