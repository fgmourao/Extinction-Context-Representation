...
hilb.parameters.filters = [3 5;7 9];
...
%% Polar PLots. Baseline and Mean Trials.

% Define data to plot
data_phase_trials             = hilb.delta_phase_trial;
data_phase_trials_mean        = hilb.delta_phase_mean_trial;
data_PLV_mean_trials          = hilb.PLV_mean_trial;


data_phase_trials_mean_session = hilb.delta_phase_mean_session  ;
data_PLV_mean_session          = hilb.PLV_mean_session ;

sgtitle_ = {'4 Hz Bandpass filter';'8 Hz Bandpass filter'};

% Baseline
for ii = 1:size(data_phase_trials,2)
    ...
    % mPFC PL <--> mPFC IL
    subplot(3,12,1)
    polarplot([zeros(1,size(data_phase_trials{1,ii}(1,1:20:end),2)) data_phase_trials{1,ii}(1,1:20:end)]',repmat([0 1],1,size(data_phase_trials{1,ii}(1,1:20:end),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_trials_mean{1,ii}(1,1)],[0 data_PLV_mean_trials{1,ii}(1,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{16} Baseline'];[];['\fontsize{14} PLV = ', num2str(data_PLV_mean_trials{1,ii}(1,1))];[]})

    % mPFC PL <--> dHPC
    subplot(3,12,13)
    polarplot([zeros(1,size(data_phase_trials{1,ii}(2,1:20:end),2)) data_phase_trials{1,ii}(2,1:20:end)]',repmat([0 1],1,size(data_phase_trials{1,ii}(2,1:20:end),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_trials_mean{1,ii}(2,1)],[0 data_PLV_mean_trials{1,ii}(2,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{14} PLV = ', num2str(data_PLV_mean_trials{1,ii}(2,1))];[]})

    % mPFC IL <--> dHPC
    subplot(3,12,25)
    polarplot([zeros(1,size(data_phase_trials{1,ii}(3,1:20:end),2)) data_phase_trials{1,ii}(3,1:20:end)]',repmat([0 1],1,size(data_phase_trials{1,ii}(3,1:20:end),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_trials_mean{1,ii}(3,1)],[0 data_PLV_mean_trials{1,ii}(3,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{14} PLV = ', num2str(data_PLV_mean_trials{1,ii}(3,1))];[]})
...
for ii = 1:size(data_phase_trials,2)
    figure(ii)
    for tt = 1:size(data_phase_trials{3,ii},3)

        % mPFC PL <--> mPFC IL
        subplot(3,12,tt+1)
        polarplot([zeros(1,size(data_phase_trials{2,ii}(1,1:20:end,tt),2)) data_phase_trials{2,ii}(1,1:20:end,tt)]',repmat([0 1],1,size(data_phase_trials{2,ii}(1,1:20:end,tt),2))','color',[.6 .6 .6]);
        hold on
        I=polarplot([0 data_phase_trials_mean{2,ii}(1,tt)],[0 data_PLV_mean_trials{2,ii}(1,tt)]);
        set(I,'linewidth',6,'color',[0 0 .6])
        thetaticks([0 90 180 270])
        title({['\fontsize{16} CS-Tone: ' num2str(tt)];[];['\fontsize{14} PLV = ' num2str(data_PLV_mean_trials{2,ii}(1,tt))];[]},'color',[0 0 .6])

        % mPFC PL <--> dHPC
        subplot(3,12,tt+13)
        polarplot([zeros(1,size(data_phase_trials{2,ii}(2,1:20:end,tt),2)) data_phase_trials{2,ii}(2,1:20:end,tt)]',repmat([0 1],1,size(data_phase_trials{2,ii}(2,1:20:end,tt),2))','color',[.6 .6 .6]);
        hold on
        I=polarplot([0 data_phase_trials_mean{2,ii}(2,tt)],[0 data_PLV_mean_trials{2,ii}(2,tt)]);
        set(I,'linewidth',6,'color',[0 0 .6])
        thetaticks([0 90 180 270])
        title({['\fontsize{14} PLV = ', num2str(data_PLV_mean_trials{2,ii}(2,tt))];[]},'color',[0 0 .6])

        % mPFC IL <--> dHPC
        subplot(3,12,tt+25)
        polarplot([zeros(1,size(data_phase_trials{2,ii}(3,1:20:end,tt),2)) data_phase_trials{2,ii}(3,1:20:end,tt)]',repmat([0 1],1,size(data_phase_trials{2,ii}(3,1:20:end,tt),2))','color',[.6 .6 .6]);
        hold on
        I=polarplot([0 data_phase_trials_mean{2,ii}(3,tt)],[0 data_PLV_mean_trials{2,ii}(3,tt)]);
        set(I,'linewidth',6,'color',[0 0 .6])
        thetaticks([0 90 180 270])
        title({['\fontsize{14} PLV = ', num2str(data_PLV_mean_trials{2,ii}(3,tt))];[]},'color',[0 0 .6])

    end

    % mPFC PL <--> mPFC IL
    subplot(3,12,12)
    polarplot([zeros(1,size(circ_mean(data_phase_trials{2,ii}(1,1:20:end,:),[],3),2)) circ_mean(data_phase_trials{2,ii}(1,1:20:end,:),[],3)]',repmat([0 1],1,size(circ_mean(data_phase_trials{2,ii}(1,1:20:end,:),[],3),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_trials_mean_session{2,ii}(1,1)],[0 data_PLV_mean_session{2,ii}(1,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{16} Averaged Trials'];[];['\fontsize{14} PLV = ', num2str(data_PLV_mean_session{2,ii}(1,1))];[]})

    % mPFC PL <--> dHPC
    subplot(3,12,24)
    polarplot([zeros(1,size(circ_mean(data_phase_trials{2,ii}(2,1:20:end,:),[],3),2)) circ_mean(data_phase_trials{2,ii}(2,1:20:end,:),[],3)]',repmat([0 1],1,size(circ_mean(data_phase_trials{2,ii}(2,1:20:end,:),[],3),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_trials_mean_session{2,ii}(2,1)],[0 data_PLV_mean_session{2,ii}(2,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{14} PLV = ', num2str(data_PLV_mean_session{2,ii}(2,1))];[]})

    % mPFC IL <--> dHPC
    subplot(3,12,36)
    polarplot([zeros(1,size(circ_mean(data_phase_trials{2,ii}(3,1:20:end,:),[],3),2)) circ_mean(data_phase_trials{2,ii}(3,1:20:end,:),[],3)]',repmat([0 1],1,size(circ_mean(data_phase_trials{2,ii}(3,1:20:end,:),[],3),2))','color',[.6 .6 .6]);
    hold on
    I=polarplot([0 data_phase_trials_mean_session{2,ii}(3,1)],[0 data_PLV_mean_session{2,ii}(3,1)]);
    set(I,'linewidth',6,'color',[0 0 0])
    thetaticks([0 90 180 270])
    title({['\fontsize{14} PLV = ', num2str(data_PLV_mean_session{2,ii}(3,1))];[]})
...
%clear('ii','I','data_phase_trials','data_phase_trials_mean','data_PLV_mean')
