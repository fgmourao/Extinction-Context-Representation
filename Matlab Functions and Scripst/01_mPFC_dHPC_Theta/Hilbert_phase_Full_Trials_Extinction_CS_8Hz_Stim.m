...
hilb.parameters.filters = [3 5;6 8;7 10];
...
for ii = 1:size(data.lfp{9,1},1)
    for tt = 1:size(CSIT{ms},2)
        for jj = 1:size(hilb.parameters.filters,1)
            ...
            temp = eegfilt(data.lfp{9,1}(ii,:,CSIT{ms}(1,tt)),parameters.decimated_srate,hilb.parameters.filters(jj,1),hilb.parameters.filters(jj,2)); % just filtering
...
for ii = 1:size(data_phase,2)
    ...
    for tt = 1:size(data_phase{3,ii},3)
        ...
        % mPFC PL <--> mPFC IL
        subplot(3,10,tt+1)
        polarplot([zeros(1,size(data_phase{3,ii}(1,1:20:end,tt),2)) data_phase{3,ii}(1,1:20:end,tt)]',repmat([0 1],1,size(data_phase{3,ii}(1,1:20:end,tt),2))','color',[.6 .6 .6]);
        hold on
        I=polarplot([0 data_phase_mean{3,ii}(1,tt)],[0 data_PLV_mean{3,ii}(1,tt)]);
        set(I,'linewidth',6,'color',[0, 0, .6])
        thetaticks([0 90 180 270])
        title({['\fontsize{16} 5 min Block ' num2str(tt)];[];['\fontsize{14} PLV = ' num2str(data_PLV_mean{4,ii}(1,tt))];[]},'color',[0, 0, .6])

        % mPFC PL <--> dHPC
        subplot(3,10,tt+11)
        polarplot([zeros(1,size(data_phase{3,ii}(2,1:20:end,tt),2)) data_phase{3,ii}(2,1:20:end,tt)]',repmat([0 1],1,size(data_phase{3,ii}(2,1:20:end,tt),2))','color',[.6 .6 .6]);
        hold on
        I=polarplot([0 data_phase_mean{3,ii}(2,tt)],[0 data_PLV_mean{3,ii}(2,tt)]);
        set(I,'linewidth',6,'color',[0, 0, .6])
        thetaticks([0 90 180 270])
        title({['\fontsize{14} PLV = ', num2str(data_PLV_mean{3,ii}(2,tt))];[]},'color',[0, 0, .6])

        % mPFC IL <--> dHPC
        subplot(3,10,tt+21)
        polarplot([zeros(1,size(data_phase{3,ii}(3,1:20:end,tt),2)) data_phase{3,ii}(3,1:20:end,tt)]',repmat([0 1],1,size(data_phase{3,ii}(3,1:20:end,tt),2))','color',[.6 .6 .6]);
        hold on
        I=polarplot([0 data_phase_mean{3,ii}(3,tt)],[0 data_PLV_mean{3,ii}(3,tt)]);
        set(I,'linewidth',6,'color',[0, 0, .6])
        thetaticks([0 90 180 270])
        title({['\fontsize{14} PLV = ', num2str(data_PLV_mean{3,ii}(3,tt))];[]},'color',[0, 0, .6])
